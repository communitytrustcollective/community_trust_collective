...
filter_text = st.caption('''Paste an attendee list, names, or WSDC numbers (one per line) to filter down to potentially relevant reports.
...
***Match results may include false positives.*** Use the available information to verify that a match refers to the correct individual before taking any action.''')

to_find = st.text_area('{filter_text}', placeholder='''Name or WSDC number
